 ...
 */
class BackupController extends PanelController
{
	...
	{
		if ( isset ($this->module->backupPath )) $this->_path = $this->module->backupPath;
		...
		if ( !file_exists($this->_path ))
		{
			if (@mkdir($this->_path )) {
				...
				throw new IOException('Unable to create backup directory '.$this->_path, IOException::FILE_SYSTEM_ERROR);
			}
		} elseif ( !is_writable($this->_path) ) {
			throw new IOException('Backup directory '.$this->_path. ' is not writable.', IOException::FILE_SYSTEM_ERROR);
		}
		return $this->_path;
	}

	private function confirmCriticalAction(){
		if ( strcasecmp($this->action->id, Yii::app()->request->getParam('action')) == 0 ) {

			// there is another value "email" here

			return true;
		} else {
			$this->render('confirm_critical_action');
			return false;
		}
	}
	...
	public function actionDelete($file = null)
	{
		...
		header('http/1.1 403');
		echo 'Please do not delete backup files.'. PHP_EOL. PHP_EOL;
		echo 'If you really want to do it, do it in command line.';
		return;
